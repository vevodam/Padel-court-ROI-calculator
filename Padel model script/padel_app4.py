...
with st.sidebar.expander("🏗 Investice a financování", expanded=True):
    ...
    investice_kurt = st.number_input("Cena za 1 kurt (Kč)", min_value=0, value=900000, step=10000)
    vlastni_kapital_proc = st.number_input("% vlastního kapitálu", min_value=0, max_value=100, value=50) / 100
...
with st.sidebar.expander("🎾 Provozní parametry", expanded=True):
    cena_hod = st.number_input("Cena za hodinu pronájmu (Kč)", min_value=0, value=250, step=10)
    oteviracka = st.number_input("Otevírací doba (hod/den)", min_value=1, max_value=24, value=14)
    obsazenost_proc = st.number_input("% obsazenost kapacity", min_value=0, max_value=100, value=60) / 100
...
# NOVÁ LOGIKA VÝNOSŮ: Kapacita * % Obsazenost * Cena
denni_kapacita_hodin = n_kurtu * oteviracka
realne_prodane_hodiny_den = denni_kapacita_hodin * obsazenost_proc
mesicni_vynos = realne_prodane_hodiny_den * cena_hod * 30.42
...
if len(idx_zvratu) > 0:
    ...
    text_zvratu = f"{mesic_zvratu} m. ({round(mesic_zvratu/12, 1)} let)"
else:
    mesic_zvratu = None
    text_zvratu = "V nedohlednu"

# --- ZOBRAZENÍ VÝSLEDKŮ ---
st.header("📊 Výsledky analýzy")

# První řada: Investice a Financování
...
if vyska_uveru > 0:
    ...
    c3.metric("Financování", "100 % vlastní")

st.markdown("---")

# Druhá řada: Klíčové indikátory
c4, c5, c6 = st.columns(3)
if npv_vysledek > 0:
    c4.metric("NPV projektu", f"{npv_vysledek:,.0f} Kč", delta="Ziskové")
else:
    c4.metric("NPV projektu", f"{npv_vysledek:,.0f} Kč", delta="Ztrátové", delta_color="inverse")
c5.metric("Bod zvratu", text_zvratu)
c6.metric("Denní prodej", f"{round(realne_prodane_hodiny_den, 1)} hod / {denni_kapacita_hodin} hod")
...
if mesic_zvratu is not None:
    ax.scatter(mesic_zvratu, 0, color='red', s=100, zorder=5)
    ax.annotate(f' Bod zvratu: {mesic_zvratu}. m', (mesic_zvratu, 0), xytext=(mesic_zvratu, 50000),
                fontweight='bold', color='red')

if vyska_uveru > 0:
    ax.axvline(doba_uveru_let * 12, color='orange', linestyle='--', alpha=0.7, label="Úvěr splacen")

ax.fill_between(mesice_osa, akumulovany_cf, 0, where=(np.array(akumulovany_cf) >= 0), color='green', alpha=0.15)
ax.fill_between(mesice_osa, akumulovany_cf, 0, where=(np.array(akumulovany_cf) < 0), color='red', alpha=0.15)

ax.set_title("Ekonomický vývoj v čase", fontsize=14)
