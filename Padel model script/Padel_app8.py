...
def datum_na_doy(d):
    """Převede datum na číslo dne v roce (1–365), ignoruje přestupné roky (29. 2. → 28. 2.)."""
    mesice = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    den = min(d.day, mesice[d.month - 1])
...
def mesicni_faktor_sezony(mesic, start_doy, konec_doy):
    ...
    mesice = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    kum = [0] + list(np.cumsum(mesice))
    d1 = kum[mesic - 1] + 1   # první den měsíce (v roce)
    d2 = kum[mesic]            # poslední den měsíce (v roce)
    dni = mesice[mesic - 1]

    def prekryv(a1, a2, b1, b2):
        return max(0, min(a2, b2) - max(a1, b1) + 1)

    if start_doy <= konec_doy:
        v_sezone = prekryv(d1, d2, start_doy, konec_doy)
    else:  # sezóna přes přelom roku
        v_sezone = prekryv(d1, d2, start_doy, 365) + prekryv(d1, d2, 1, konec_doy)
    return v_sezone / dni
...
with st.sidebar.expander("⏳ Horizont a provoz", expanded=True):
    ...
    with col_s:
        sezona_start = st.date_input("Začátek sezóny", value=datetime.date(2025, 4, 1))
    with col_e:
        sezona_konec = st.date_input("Konec sezóny", value=datetime.date(2025, 10, 31))
    _start_doy = datum_na_doy(sezona_start)
    _konec_doy = datum_na_doy(sezona_konec)
    if _start_doy <= _konec_doy:
        _delka_dni = _konec_doy - _start_doy + 1
    else:
        _delka_dni = 365 - _start_doy + 1 + _konec_doy
    st.info(f"Délka sezóny: **{_delka_dni} dní** ({round(_delka_dni / 365 * 12, 1)} měs. ekvivalent)")
    horizont_let = st.number_input("Doba sledování (roky)", min_value=1, max_value=30, value=10)

# --- VÝPOČTY ---
sezona_start_doy = datum_na_doy(sezona_start)
sezona_konec_doy = datum_na_doy(sezona_konec)
...
if vyska_uveru > 0:
    ...
    if r_mesicni == 0:
        mesicni_splatka = vyska_uveru / n_splatek
    else:
...
for m in range(1, mesicu + 1):
    ...
    # Aplikace sezónnosti — proporcionální podíl dní měsíce v sezóně
    faktor_sezony = mesicni_faktor_sezony(mesic_v_roce, sezona_start_doy, sezona_konec_doy)
    vynos = mesicni_vynos_total * faktor_sezony
