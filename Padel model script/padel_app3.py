...
c4, c5 = st.columns(2)
if npv_vysledek > 0:
    c4.metric("NPV projektu (ziskovost)", f"{npv_vysledek:,.0f} Kč", delta="Ziskové")
else:
    c4.metric("NPV projektu (ziskovost)", f"{npv_vysledek:,.0f} Kč", delta="Ztrátové", delta_color="inverse")
c5.metric("Bod zvratu (Návratnost)", text_zvratu)
...
if mesic_zvratu is not None:
    ax.scatter(mesic_zvratu, 0, color='red', s=100, zorder=5, label="Bod zvratu")
    ax.annotate(f' Bod zvratu: {mesic_zvratu}. m', (mesic_zvratu, 0), xytext=(mesic_zvratu, 50000),
                fontweight='bold', color='red')

# Svislá čára pro úvěr
if vyska_uveru > 0:
