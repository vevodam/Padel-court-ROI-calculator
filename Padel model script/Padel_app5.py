...
with st.sidebar.expander("💰 Cenotvorba (Denní model)", expanded=True):
    st.subheader("Špička (Prime Time)")
    cena_spicka = st.number_input("Cena ve špičce (Kč/h)", value=600)
    hodin_spicka = st.number_input("Počet hodin špičky/den", value=6)
    ...
    # Zbytek do 14 hodin otevíračky
    hodin_mimo = 14 - hodin_spicka
    st.info(f"Mimo špičku zbývá {hodin_mimo} hod/den")
...
current_opex = opex_mesic_start

for m in range(1, mesicu + 1):
    # Určení aktuálního roku (pro dynamický pricing)
    aktualni_rok = (m - 1) // 12

    # Eskalace cen a nákladů (každý rok se zvýší o zadané procento)
    ...
    # Denní tržba (vážený průměr špičky a mimo špičku)
    denni_trzba_spicka = n_kurtu * hodin_spicka * obsazenost_spicka * cena_spicka_akt
    denni_trzba_mimo = n_kurtu * hodin_mimo * obsazenost_mimo * cena_mimo_akt
...
c4.metric("Návratnost", f"{mesic_zvratu} m." if mesic_zvratu else "Nedohledno")
...
# --- INFORMAČNÍ TABULKA ---
with st.expander("🔍 Detail cenotvorby v čase"):
    st.write(f"Počáteční denní tržba v sezóně: {round(denni_trzba_spicka + denni_trzba_mimo, 0):,.0f} Kč")
    st.write(
        f"Cena ve špičce v posledním roce ({horizont_let}. rok): {round(cena_spicka * (1 + narust_ceny_rok) ** (horizont_let - 1), 0):,.0f} Kč")