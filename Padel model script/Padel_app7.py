...
#Tento skript místo matplotlib využívá plotly - graf je interaktivní
#dále byla provedena změna ve výpočtu NPV
...
# Hlavní křivka kumulativního CF
fig.add_trace(go.Scatter(
    x=list(range(mesicu + 1)),
    y=akumulovany_cf,
    mode='lines',
    name='Kumulativní Cash Flow',
    line=dict(color='dodgerblue', width=3),
    fill='tozeroy',
    fillcolor='rgba(30, 144, 255, 0.1)'
))

# Zvýraznění bodu zvratu
if mesic_zvratu:
    fig.add_trace(go.Scatter(
        x=[mesic_zvratu],
        y=[0],
...
# Formátování grafu
fig.update_layout(
    title="Ekonomický vývoj s dynamickým pricingem",
    xaxis_title="Měsíce provozu",
    yaxis_title="Zůstatek (CZK)",
    hovermode="x unified",
    showlegend=True,
